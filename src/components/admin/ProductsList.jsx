...
import { Button, Checkbox, Tooltip, Image } from "antd";
...
const ProductsList = ({ data }) => {
  ...
  const dataFirebase = useSelector((state) => state.crud.data);
  const [products, setProducts] = useState();

  const loadData = async () => {
    await dispatch(findAllData(data));
  };
  ...
  const checkboxOptions = products?.map((product, index) => ({
    label: (
      <>
        <div
          key={index}
          className="flex items-center justify-between border-s text-center flex-1 w-full"
        >
          ...
          <Tooltip title="Giá bán">
            <div className=" py-3 w-[10%]">{formatPrice(product.price)}</div>
          </Tooltip>
          <Tooltip title="Giảm giá">
            <div className=" py-3 w-[10%]">{product.sale}</div>
          </Tooltip>
          <Tooltip title="Giá sau giảm">
            <div className=" py-3 w-[10%]">{formatPrice(product.price)}</div>
          </Tooltip>

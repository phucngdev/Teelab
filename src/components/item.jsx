...
const Item = () => {
  // lấy id product và render apis
  const { id } = useParams();
  const product = TLAT.find((p) => p.id === id);
  ...
  // in iconName vào Màu sắc trong phần mô tả
  const joinedIconNames = product.imgIcon
    .map((name) => name.iconName)
    .join("/"); // nối với nhau bằng /

  // slide ảnh
  const showSlide = product.imgItem.map((img, index) => (
    <div
      key={index}
      className="h-[360px] md:h lg:h-[500px] flex justify-center items-center"
    >
      <img className="object-cover h-full" src={img.item} alt="" />
    </div>
  ));
  return (
    product && (
            ...
            <div className="flex flex-col md:flex-row sm:justify-between sm:flex-wrap">
              ...
              <div className="w-full lg:w-[40%] flex-grow-0 flex-shrink-0 px-[15px]">
                ...
                <form action="">
                  ...
                  <button
                    type="submit"
                    className="h-[47px] w-full rounded-md bg-black mt-[15px] text-white uppercase "
                  >
                    THÊM VÀO GIỎ HÀNG
                  </button>
                </form>
              ...
              <div className="my-[30px]">
                ...
                <div className="py-[15px]">
                  ...
                  <p className="text-[#333] text-sm leading-6">
                    ...
                    {/* in màu sắc trong imgIcon */}- Màu sắc: {joinedIconNames}{" "}
                    <br />
